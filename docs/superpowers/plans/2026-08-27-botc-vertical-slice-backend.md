...
- **The Demon's bluffs can number fewer than 3, or zero, in a game that deals every currently-implemented good character.** With only four good characters in this vertical slice (Washerwoman, Empath, Soldier, Butler), a standard 6-player game (3 Townsfolk + 1 Outsider) uses all four, leaving nothing in the not-in-play pool for `demonInfo.js`'s bluff selection. `goodPool.slice(0, 3)` degrades safely (fewer or zero entries, never a throw), so this is not a crash risk — but it is a real gameplay gap, unfixable within this plan's scope since it requires more good characters to exist. It resolves naturally once the character-library follow-up plan adds more Townsfolk/Outsiders.
