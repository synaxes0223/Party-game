...
- **A dead player's spent ghost vote can silently stall `maybePromptVoteTurn` for the rest of a nomination.** `voting.js`'s `castVote` (unchanged, out of this plan's file scope) rejects — without advancing `currentVoterIndex` — when the seat at that position is dead and has already used its one lifetime ghost vote. Once that happens, `nomination.order[currentVoterIndex]` permanently points at that ineligible seat for the rest of the current nomination: `maybePromptVoteTurn` keeps lighting up that dead player's phone for a vote they can never cast, and every alive player later in the seating order loses their own automatic vote-turn prompt for that nomination, falling back to Storyteller-manual-entry (`host:botc-vote`) with no signal that this happened. This is a pre-existing quirk in `voting.js`'s vote-pointer bookkeeping that this plan's new prompt feature is the first to make visible to real players — it doesn't corrupt state or block the game (the Storyteller's full manual override remains available, per the spec's "always manually overridable" governing principle), so it's not addressed here. A proper fix belongs in a follow-up touching `voting.js`: make `currentVoterIndex` skip ineligible (dead-and-vote-spent) seats, the same way `nightLoop.js`'s `skipToSchedulable` already does for the night order.
