...
- **Cross-reference (not a new gap): the ghost-vote-stall backend limitation.** `voting.js`'s vote-pointer bookkeeping (unchanged by this plan) can leave `currentVoterIndex` stuck on a dead, ghost-vote-spent seat for the rest of a nomination — already documented as a known limitation in the prior `2026-08-27-botc-player-driven-turns.md` plan, which added the player-driven vote-turn prompts that first made it visible. Task 7's day panel inherits this exactly as `host:botc-vote`/`host:botc-resolve-vote` already did: the tally would show Yes/No buttons for that seat, and clicking either would be silently rejected server-side. Still degrades gracefully to the Storyteller's full manual override (any OTHER seat's vote can still be cast, and the nomination can be abandoned/re-nominated); the actual fix belongs in a `voting.js` follow-up, out of scope for both this plan and the one before it.
